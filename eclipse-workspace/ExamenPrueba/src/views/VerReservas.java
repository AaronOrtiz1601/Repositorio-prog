...
/**
 * Clase que se encarga de mostrar la reserva de clases que hay 
 */
public class VerReservas extends JDialog {
  //Atributos privados de la clase
    ...
    public static void main(String[] args) {
        try {
          VerReservas dialog = new VerReservas();
    ...
    public VerReservas() {
        ...
        String[] columnNames = {"Nombre", "Apellidos", "Class", "turno"};
     ...
     // Agrega el JScrollPane al panel de contenido
        contentPanel.add(scrollPane); 

        JLabel lblNewLabel = new JLabel("Ver reservas");
    ...
    private void actualizarTabla() {
        ...
        // Limpia el modelo antes de agregar filas
        tableModel.setRowCount(0); 

        // Iterar sobre los clientes y agregar los datos al modelo
        for (Reservas reservas : App.reservas) {
            String[] rowData = {
                reservas.getNombre(),
                reservas.getApellido(),
                reservas.getNombreClase(), 
                reservas.getTurno()
            };
    ...
    private void filtrarTabla() {
        String clases = textFieldApellidos.getText().toLowerCase();

        // Limpiar el modelo de la tabla
        tableModel.setRowCount(0);

        // Iterar sobre los clientes y agregar los que coincidan con los filtros
        for (Reservas reserva : App.reservas) {
            boolean matches = true;

            if (!clases.isEmpty() && !reserva.getNombreClase().toLowerCase().contains(clases)) {
                matches = false;
            }
